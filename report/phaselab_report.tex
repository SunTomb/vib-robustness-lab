...
系统根据 clean accuracy drop、CBI 和 KL collapse score 给每个配置赋予 `under-regularized`、`useful-compression`、`robustness-specialized`、`over-compressed` 或 `unstable` 标签。该标签是实验诊断工具，不是理论定理。
...
后端位于 `backend/`，使用 FastAPI 只读 artifacts，不执行训练。主要接口包括 `/api/index`、`/api/experiments/{id}`、`/api/phase/index`、`/api/phase/summary` 和 `/api/phase/experiments/{id}`。前端位于 `frontend/`，使用 React、TypeScript、Vite 和 Recharts，包含 Phase overview、phase map、Compression Lens、Diagnosis Lab 和原始 VIB 实验台。
