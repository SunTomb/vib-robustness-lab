...
PhaseLab 关注的问题是：VIB 的压缩何时是 useful compression，何时变成 destructive over-compression。实验以 KL(q(z|x)||p(z)) 作为 I(X;Z) 的变分上界 proxy，以 clean accuracy 和 corruption accuracy 作为任务信息与鲁棒性的经验 proxy。CNN baseline 单独标记为 baseline，用作参考而不解释为 VIB 压缩阶段。
...
Phase label 分布：`{'baseline': 1, 'under-regularized': 4, 'useful-compression': 6, 'unstable': 1, 'over-compressed': 1}`。
...
Phase label 分布：`{'baseline': 1, 'under-regularized': 2, 'useful-compression': 2, 'unstable': 7, 'over-compressed': 1}`。
...
Phase label 分布：`{'baseline': 1, 'under-regularized': 1, 'useful-compression': 6, 'unstable': 4, 'over-compressed': 1}`。
